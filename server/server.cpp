...
//	cout << "Awake!" << endl;
	...
	const string file_root_name = data_path + file_name.substr(0, lastIndex);
	string extension;
	...
	*/
	...
	// Instead, assume we're in the repo root and find file in sample_videos
	const string output_file = "sample_videos/" + path_elems[1];
	...
	cout << file_root_name << ".jpg successfully generated" << endl;
	http_response response (status_codes::OK);
	response.headers().add(U("Access-Control-Allow-Origin"), U("*"));

	// Let the client know
	json::value response_json;
//	response_json["file_path"] = json::value("\"" + file_root_name + ".jpg" + "\"");
...
void bgExtractionServer::get_cb(http_request request){
	...
	// Asking for background
		...
		if (loadFileToString(background_file, background_data)){
			http_response response (status_codes::OK);
			response.headers().add(U("Access-Control-Allow-Origin"), U("*"));
			response.set_body(background_data, "image/jpeg");
			request.reply(response);
			return;
		}
		else {
